...
int main(void) {
    ...
    assert(pangram("abc, def! ghi... jkl25; mnopqrstuvwxyz") == 1); // ignore punctuation
    ...
    assert(pangram("\1bcdefghijklmnopqrstuvwxyz") == 0); // close-match control code should not be false positive
