...
sigset_t set = {};
void block_sigint(void) { if (sigprocmask(SIG_BLOCK, &set, NULL)) error(); }
void unblock_sigint(void) { if (sigprocmask(SIG_UNBLOCK, &set, NULL)) error(); }
...
void setup_handler(void (*handler)(int)) { if (SIG_ERR == signal(SIGINT, handler)) error(); }
...
char **parse_cmd(char *const line) {
    char **cmd;
    if (NULL == (cmd = calloc(sizeof(char *),INIT_ARG_CNT))) error();
    ...
    for (char *token = strtok(line, delim);
        ...
        if (n == sz && NULL == (cmd = reallocarray(cmd, sizeof(char *), sz <<= 1))) error();
